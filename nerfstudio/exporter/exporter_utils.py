...
def generate_spherical_harmonics_cloud(
    pipeline: Pipeline,
    ...
    CONSOLE.print("HELLO WORLD")
    CONSOLE.print("HELLO WORLD")
    #TESTING FORCING POINT SAMPLING AT WITH A SPECIFIC DIRECTION
    from nerfstudio.cameras.rays import RaySamples
    from nerfstudio.model_components.ray_samplers import UniformSampler

    #Probably can get away with thousands of samples > 10K
    num_directions = 1024
    results = []
    grid_res = 16
    ...
    with progress as progress_bar:
        ...
        for i in range(num_directions):
            ...
            random_vector = torch.rand((1,3), device=pipeline.device)
            ...
            #sampler = UniformSampler(num_samples = 1, train_stratified=False)
            #ray_samples = sampler(bundle)
            with torch.no_grad():
    ...
    #END TESTING FORCING POINT SAMPLING AT WITH A SPECIFIC DIRECTION
    import open3d as o3d
    ...
    with progress as progress_bar:
        task = progress_bar.add_task("Generating Spherical Harmonics Cloud", total=num_points)
        while not progress_bar.finished:
            ...
            with torch.no_grad():
                ...
                #Create a new ray bundle: 
                outputs = pipeline.model(ray_bundle)
                #Look at (nerfacto.py:298 get_outputs(raybundle) base_field.py:114 forward(ray_samples) and  nerfacto_field.py:230 get_output(ray_samples, density_embedding))
                #colors = pipeline.model.field.mlp_head(samples)
    ...
    ind = None
    if remove_outliers:
        CONSOLE.print("Cleaning Spherical Harmonics Cloud")
        pcd, ind = pcd.remove_statistical_outlier(nb_neighbors=20, std_ratio=std_ratio)
        print("\033[A\033[A")
        CONSOLE.print("[bold green]:white_check_mark: Cleaning Spherical Harmonics Cloud")
        if ind is not None:
    ...
    if estimate_normals:
        if normal_output_name is not None:
            ...
            sys.exit(1)
        CONSOLE.print("Estimating Spherical Harmonics Cloud Normals")
        pcd.estimate_normals()
        print("\033[A\033[A")
        CONSOLE.print("[bold green]:white_check_mark: Estimating Spherical Harmonics Cloud Normals")
